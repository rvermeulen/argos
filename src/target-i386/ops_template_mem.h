...
void OPPROTO glue(glue(op_rol, MEM_SUFFIX), _T0_T1_cc)(void)
{
	...
	glue(ARGOS_ST, MEM_SUFFIX)(A0, T0, T0TAG);
#else
...
void OPPROTO glue(glue(op_shld, MEM_SUFFIX), _T0_T1_im_cc)(void)
{
    ...
    if (count > 16)
        res |= T1 << (count - 16);
    T0 = res >> 16;
    glue(ARGOS_REG_SHLD, SUFFIX)(T0TAG, T1TAG, count);
#ifdef MEM_WRITE
    //glue(st, MEM_SUFFIX)(A0, T0);
    glue(ARGOS_ST, MEM_SUFFIX)(A0, T0, T0TAG);
#endif
...
void OPPROTO glue(glue(op_shrd, MEM_SUFFIX), _T0_T1_im_cc)(void)
{
    ...
    if (count > 16)
        res |= T1 << (32 - count);
    T0 = res;
    glue(ARGOS_REG_SHRD, SUFFIX)(T0TAG, T1TAG, count);
#ifdef MEM_WRITE
    //glue(st, MEM_SUFFIX)(A0, T0);
    glue(ARGOS_ST, MEM_SUFFIX)(A0, T0, T0TAG);
#endif
...
void OPPROTO glue(glue(op_cmpxchg, MEM_SUFFIX), _T0_T1_EAX_cc)(void)
{
	...
	glue(ARGOS_REG_MOV, SUFFIX)(EAXTAG, T0TAG);
    }
    CC_SRC = src;
    CC_DST = dst;
    FORCE_RET();
