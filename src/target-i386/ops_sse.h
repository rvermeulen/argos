...
void OPPROTO glue(op_psrlw, SUFFIX)(void)
{
    ...
    ARGOS_MMX_CLReq(&(d->Q(0)));
#if SHIFT == 1 
    ARGOS_MMX_CLReq(&(d->Q(1)));
#endif
    FORCE_RET();
}
...
void OPPROTO glue(op_psrldq, SUFFIX)(void)
{
    ...
    for(i = 0; i < 16 - shift; i++) {
        d->B(i) = d->B(i + shift);
	ARGOS_MMX_MOVeb(&(d->B(i)), &(d->B(i + shift)));
    }
    for(i = 16 - shift; i < 16; i++) {
        d->B(i) = 0; 
	ARGOS_MMX_CLReb(&(d->B(i))); 
    }
    FORCE_RET();
}
...
void OPPROTO glue(op_movl_mm_T0, SUFFIX) (void)
{
    ...
    d = (Reg *)((char *)env + PARAM1);
    d->L(0) = T0;
    ARGOS_MMX_MOVelREG(&(d->L(0)), T0TAG);
    d->L(1) = 0;
    ARGOS_MMX_CLRel(&(d->L(1)));
#if SHIFT == 1
    d->Q(1) = 0;
    ARGOS_MMX_CLReq(&(d->Q(1)));
#endif
...
void OPPROTO glue(op_movl_T0_mm, SUFFIX) (void)
{
    ...
    T0 = s->L(0);
    ARGOS_MMX_MOVREGel(T0TAG, &(s->L(0))); 
}
...
void OPPROTO glue(op_movq_mm_T0, SUFFIX) (void)
{
    ...
    d = (Reg *)((char *)env + PARAM1);
    d->Q(0) = T0;
    ARGOS_MMX_MOVrqREG(&(d->Q(0)), T0TAG);
#if SHIFT == 1
    d->Q(1) = 0;
    ARGOS_MMX_CLReq(&(d->Q(1)));
#endif
...
void OPPROTO glue(op_movq_T0_mm, SUFFIX) (void)
{
    ...
    s = (Reg *)((char *)env + PARAM1);
    T0 = s->Q(0);
    ARGOS_MMX_MOVREGeq(T0TAG, &(s->Q(0)));
}
#endif
...
void OPPROTO glue(op_pinsrw, SUFFIX) (void)
{
    ...
    d->W(pos) = T0;
    ARGOS_MMX_MOVewREG(&(d->W(pos)), T0TAG);
}
...
void OPPROTO glue(op_pextrw, SUFFIX) (void)
{
    ...
    T0 = s->W(pos);
    ARGOS_MMX_MOVREGew(T0TAG, &(s->W(pos)));
}
